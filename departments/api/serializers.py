...
class DepartmentSerializers(serializers.ModelSerializer):
    ...
    def create(self , validated_data):
        ...
        department_name = dept_name.strip().lower()
        user = self.context['request'].user
        teacher_ids = validated_data.get('teachers' , [])

        if Department.objects.filter(name__iexact=department_name , is_active=True).exists():
            raise serializers.ValidationError({"Duplication Error": f"Department with similar name '{dept_name}' already exists"})
    ...
    def update(self, instance, validated_data):
        dept_name = validated_data.get('name', instance.name)
        new_teacher_ids = validated_data.get('teachers')
        
        # Check if the department name is changing and whether the new name already exists
        if (instance.name).strip().lower() != (dept_name).strip().lower():
            if Department.objects.filter(name__iexact=dept_name, is_active=True).exclude(id=instance.id).exists():
                raise serializers.ValidationError({"Duplication Error": f"Department with similar name '{dept_name}' already exists"})
        
        # If 'teachers' field is in validated data, process teacher updates
        if new_teacher_ids is not None:
            new_teacher_ids = set(new_teacher_ids)
            # Validate teachers if 'teachers' data is provided
            ...
            # If 'teachers' field is not in validated_data, only update the department name
            instance.name = dept_name
            instance.save()
    ...
    def delete(self , instance):
        ...
        for teacher_id in teachers:
            teacher = CustomUser.objects.get(id=teacher_id)
            instance.teacher.remove(teacher)
        Thread(target=deletedDepartmentRelatedStuff, args=(instance.id,)).start()
